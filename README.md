...
## Индексы
В проекте предусмотрено хранение отраслевых индексов. Значения можно добавить вручную в файл
`data/indices_manual.csv` и импортировать командой `mmw.indices.import_indices_from_csv`.

Для справки:
- **HARPEX** — <https://harpex.harperpetersen.com/>
- **SCFI** — <https://en.sse.net.cn/indices/>
- **WCI** — <https://www.drewry.co.uk/supply-chain-advisors/supply-chain-expertise/world-container-index>
- **FBX** — <https://fbx.freightos.com/>

Существуют экспериментальные парсеры этих страниц. Их следует использовать только если это не нарушает
условия использования или ограничения `robots.txt`. В противном случае данные рекомендуется добавлять
вручную через CSV.
